            ...
            if (request.method === 'GET') {
              ...
              if (sender) {
                ...
                context.res = {
                  ...
                  const senderID = request.query.get('senderID');
                  context.log(senderID);
                  const usersPendingRespond = await queryDB(connection, 'SELECT * FROM users_pending_respond WHERE sender = ? and waiting=1', senderID);
                  context.res = {
                      body: JSON.stringify(usersPendingRespond),
                  };
              }  
                ...
                try {
                  ...
                  if (requestData.waiting !== undefined) {
                    // Nueva ruta para guardar usuarios pendientes de respuesta
                    const result = await saveUserPendingRespond(connection, requestData);
                    context.res = {
                        body: {
                            message: 'Usuario pendiente de respuesta registrado satisfactoriamente',
                            id: result.insertId,
                        },
                    };
                  } else {
...
async function getChatsWithMessages(connection, sender) {
    try {
        ...
        const chatsResult = await connection.query('SELECT date, id FROM chats WHERE sender = ? order by id', [sender]);
        ...
        for await(const chat of chatsResult) {
            ...
            const messagesResult = await connection.query('SELECT role, content FROM messages WHERE chat_id = ? order by id', [chat.id]);
...
async function saveUserPendingRespond(connection, values) {
  const insertQuery = 'INSERT INTO users_pending_respond (sender, waiting) VALUES (?, ?)';
  return new Promise((resolve, reject) => {
      connection.query(insertQuery, [values.sender, values.waiting], (err, results) => {
          if (err) {
              console.error('Error al ejecutar la consulta SQL:', err);
              reject(err);
          } else {
              console.log('Usuario pendiente de respuesta insertado con éxito:');
              resolve(results);
          }
      });
  });
